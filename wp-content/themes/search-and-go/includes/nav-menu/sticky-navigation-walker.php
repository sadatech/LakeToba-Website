 ...
 */
if (!class_exists('SearchAndGoStickyNavigationWalker')) {
	class SearchAndGoStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				$item_output .= '<span class="item_inner" style="text-align: center">';
				...
				$item_output .= '<span class="menu_icon_wrapper"><i class="menu_icon '.$icon.'"></i></span><br>';
				...
				if($args->has_children && $angle_icon != "") {
					$item_output .= '<i class="eltd-menu-arrow fa '.$angle_icon.'"></i>';
				}
