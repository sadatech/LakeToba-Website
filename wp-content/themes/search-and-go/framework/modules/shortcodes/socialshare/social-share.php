<?php
namespace SearchAndGoElated\Modules\Shortcodes\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		vc_map(array(
			'name' => esc_html__('Elated Social Share', 'search-and-go'),
			'base' => $this->getBase(),
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (search_and_go_elated_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (search_and_go_elated_options()->getOptionValue('enable_social_share_on_'.$post_type)) ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= search_and_go_elated_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (search_and_go_elated_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (search_and_go_elated_options()->getOptionValue($net.'_icon')) ? search_and_go_elated_options()->getOptionValue($net.'_icon') : '';
				$html = search_and_go_elated_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
		...
		switch ($net) {
			case 'facebook':
				if(wp_is_mobile()) {
					...
					$link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(search_and_go_elated_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '&amp;p[summary]=' . urlencode(search_and_go_elated_addslashes(get_the_excerpt())) . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
				}
				break;
			case 'twitter':
				$count_char = (isset($_SERVER['https'])) ? 23 : 22;
				$twitter_via = (search_and_go_elated_options()->getOptionValue('twitter_via') !== '') ? ' via ' . search_and_go_elated_options()->getOptionValue('twitter_via') . ' ' : '';

				if(wp_is_mobile()) {
					$link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(search_and_go_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
				} else {
					$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(search_and_go_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				}
				break;
			case 'google_plus':
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . search_and_go_elated_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
