...
class ElatedInstagramApi {
     ...
     */
    public function getImages($limit = '', $tag = '', $user_id = '', $transient = array()) {
        $response = $this->fetchData($limit, $tag, $user_id, $transient);
     ...
     *
     * @param int|string $limit how much images to retrieve
     * @param string $tag from which tag to retrieve images
     * @param string $user_id
     * @param array $transient transient config
     *
     ...
     */
    private function fetchData($limit = '', $tag = '', $user_id = '', $transient = array('use_transient' => false)) {
        ...
        if($this->useTransients($transient) && get_transient($transient['transient_name'])) {
            ...
            if($tag !== '') {
                ...
                //request data from API
                $httpResponse = wp_remote_get($this->apiURL.'/users/' . $user_id . '/media/recent/?access_token='.$this->accessToken.'&count='.$limit);
            } else {
                //request data from API
    ...
    public function getAuthorizeUrl() {
        return 'https://api.instagram.com/oauth/authorize?client_id='.$this->getClientID().'&redirect_uri='.$this->getRedirectUri().'&response_type='.$this->getResponseType().'&scope=public_content';
    }

    /**
     * Get ID of any user using his username
     * @param $username
     *
     * @return string
     */
    public function getUserIDFromUsername( $username ) {
        ...
        $httpResponse = wp_remote_get($this->apiURL.'/users/search?q="'. $username .'"&access_token='.$this->accessToken.'&scope=public_content');
